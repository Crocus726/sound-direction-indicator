import os, urllib.request, struct, time, sys, threading # serial, glob 모듈 뺌
import numpy as np
...
# UDP 소켓 생성 및 바인딩
UDP_IP = "0.0.0.0" # 라즈베리 파이의 IP 주소(0.0.0.0일 시 모든 인터페이스에서 수신)
UDP_PORT = 115 # UDP 통신 포트 번호
sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM) # UDP 소켓 생성(sock)
sock.bind((UDP_IP, UDP_PORT)) # 소켓에 IP 주소와 포트 번호 바인딩

#BAUD=921600 # 시리얼 통신 속도여서 뺌
#HDR_MAGIC=b"YAWAV1M"
PKT_MAGIC=b"KPAY"
...
class UDPReader(threading.Thread):
    def __init__(self, port, maxlen_s=4):
        ...
        self.F = NpRing(self.sr * maxlen_s) # Front 채널
        self.B = NpRing(self.sr * maxlen_s) # Back 채널
        self.L = NpRing(self.sr * maxlen_s) # Left 채널
        self.R = NpRing(self.sr * maxlen_s) # Right 채널
        self.ok = False
        
        try:
            self.sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            self.sock.bind(("0.0.0.0", port)) # 모든 IP로부터 해당 포트로 오는 데이터 수신
            print(f"✅ UDP 서버가 {port}번 포트에서 시작되었습니다.")
            self.ok = True
        except OSError as e:
            print(f"❌ 에러: UDP 포트 {port}를 열 수 없습니다. 다른 프로그램이 사용 중일 수 있습니다. ({e})")
            self.ok = False
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                ip_addr = addr[0]
                if ip_addr == ESP32_IPS["front_back"]:
                    if sid == 0: self.F.append(payload)
                    elif sid == 1: self.B.append(payload)
                elif ip_addr == ESP32_IPS["left_right"]:
...
def main():
    ...
    try:
        while True:
            ...
            chF = reader.F.latest(win_src)
            chB = reader.B.latest(win_src)
            chL = reader.L.latest(win_src)
            chR = reader.R.latest(win_src)
